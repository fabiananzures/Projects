...
def get(hostname, port, path="", guard_address=None, middle_address=None, exit_address=None):
    ...
    # Pick nodes to form your circuit, an ID,
    ...
    guard_middle_nodes = get_all_relays(consensus)
    exit_nodes = get_all_exits(consensus)
    circuit_id = gen_circuit_id()
    
    if guard_address == None and middle_address == None and exit_address == None:
        guard_router = random_router(guard_middle_nodes)
        guard_middle_nodes.remove(guard_router)
        middle_router = random_router(guard_middle_nodes)
        exit_router = random_router(exit_nodes)
    else:
    ...
    request = b'GET /%s HTTP/1.0\r\nHost:%s\r\nPort:%s\r\n\r\n' % (path.encode(), hostname.encode(), port.to_bytes(2, byteorder='big'))  #your-code-here#
...
if __name__ == '__main__':
    ...
    register_logger(verbose=0, log_file= 'log_10.log')
    try:
        main()
